...
MV2_USE_CUDA=1 MV2_ENABLE_AFFINITY=0 MV2_SMP_CMA=0 srun  --mpi=pmi2  --gres=gpu:4 -n 1 -p Single --job-name=text_detect $CAFFEBIN train --solver=./deform_solver.pt --weights=./snapshot/pre_CNN_iter_10000.caffemodel 2>&1| tee ./logs/deform_train_$time.log &
