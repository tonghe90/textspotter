...
predict_path = '/mnt/lustre/pocgroup/dataset/research/ICDAR2017MLT/ch8_validation_images_pred'
ground_truth_path = '/mnt/lustre/pocgroup/dataset/research/ICDAR2017MLT/ch8_validation_localization_transcription_gt_v2/'
error_save_dir = '/mnt/lustre/pocgroup/dataset/research/ICDAR2017MLT/error'
...
def get_info_ground_truth(info_file, info_dict, predict_dict):
	...
	with open(info_file) as fr:
		for line in fr:
			...
			y1,x2,y2,x3,y3,x4,y4 = map(float, line.strip().split(',')[1:8])
	...
	for predict_bbox in predict_list:
		...
		for i in range(len(ground_truth_list)):
			if ground_truth_unuse[i]:
				try:
					iou = compute_iou(predict_bbox, ground_truth_list[i])
				except:
					iou = 0
				if iou>iou_threshold:
