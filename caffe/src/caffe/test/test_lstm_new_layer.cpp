...
class LSTMNewLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  LSTMNewLayerTest() : num_output_(7) {
...
TYPED_TEST_CASE(LSTMNewLayerTest, TestDtypesAndDevices);

TYPED_TEST(LSTMNewLayerTest, TestSetUp) {
  typedef typename TypeParam::Dtype Dtype;
  LSTMNewLayer<Dtype> layer(this->layer_param_);
...
TYPED_TEST(LSTMNewLayerTest, TestForward) {
  typedef typename TypeParam::Dtype Dtype;
  ...
  shared_ptr<LSTMNewLayer<Dtype> > layer(new LSTMNewLayer<Dtype>(this->layer_param_));
  ...
  layer.reset(new LSTMNewLayer<Dtype>(this->layer_param_));
  Caffe::set_random_seed(1701);
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
...
TYPED_TEST(LSTMNewLayerTest, TestLSTMUnitSetUp) {
  typedef typename TypeParam::Dtype Dtype;
...
TYPED_TEST(LSTMNewLayerTest, TestLSTMUnitGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  LSTMUnitLayer<Dtype> layer(layer_param);
...
TYPED_TEST(LSTMNewLayerTest, TestLSTMUnitGradientNonZeroCont) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  LSTMUnitLayer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
...
TYPED_TEST(LSTMNewLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LSTMNewLayer<Dtype> layer(this->layer_param_);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
...
TYPED_TEST(LSTMNewLayerTest, TestGradientNonZeroCont) {
  typedef typename TypeParam::Dtype Dtype;
  LSTMNewLayer<Dtype> layer(this->layer_param_);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
...
TYPED_TEST(LSTMNewLayerTest, TestGradientNonZeroContBufferSize2) {
  typedef typename TypeParam::Dtype Dtype;
...
TYPED_TEST(LSTMNewLayerTest, TestGradientNonZeroContBufferSize2WithStaticInput) {
  typedef typename TypeParam::Dtype Dtype;
  this->ReshapeBlobs(2, 2);
  FillerParameter filler_param;
  UniformFiller<Dtype> filler(filler_param);
