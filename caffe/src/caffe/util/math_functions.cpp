...
template <>
void caffe_cpu_bound<double>(const int N, const double* a, const double min, 
    const double max, double* y) {
  for (int i = 0; i < N; ++i) {
    double tmp = a[i];
    if (tmp > max) y[i] = max;
    else if (tmp < min) y[i] = min;
    else y[i] = tmp;
  }
}

template <>
void caffe_cpu_bound<float>(const int N, const float* a, const float min, 
    const float max, float* y) {
  for (int i = 0; i < N; ++i) {
    float tmp = a[i];
    if (tmp > max) y[i] = max;
