...
// LiangDing Add 2017/1/25
template <typename Dtype>
__global__
void bound_kernel(const int N, const float *a, const float min,
    const float max, float* y)
{
  CUDA_KERNEL_LOOP(idx, N) {
    if (a[idx] > max) y[idx] = max;
    else if (a[idx] < min) y[idx] = min;
    else y[idx] = a[idx];
  }
}
...
template <typename Dtype>
__global__
void bound_kernel(const int N, const double *a, const double min,
...
template <>
void caffe_gpu_bound<float>(const int N, const float* a, const float min, const float max, float* y) {
  bound_kernel<float><<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(
            N, a, min, max, y);
}

template <>
void caffe_gpu_bound<double>(const int N, const double* a, const double min, const double max, double* y) {
